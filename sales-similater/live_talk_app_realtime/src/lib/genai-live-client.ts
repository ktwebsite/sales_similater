...
export interface LiveClientEventTypes {
  ...
  open: () => void;
  // Emitted when output audio transcription is received
...
export class GenAILiveClient extends EventEmitter<LiveClientEventTypes> {
  ...
  protected async onmessage(message: LiveServerMessage) {
    ...
    if (message.serverContent) {
      ...
      // 入力音声の文字起こしを処理（transcriptionオブジェクトとして）
      if ("inputTranscription" in serverContent) {
        const transcription = (serverContent as any).inputTranscription;
        if (transcription?.text) {
          this.emit("inputTranscription", transcription.text);
          this.log(`server.transcription`, `input: ${transcription.text}`);
        }
      }

      // 出力音声の文字起こしを処理（transcriptionオブジェクトとして）
      if ("outputTranscription" in serverContent) {
        const transcription = (serverContent as any).outputTranscription;
        if (transcription?.text) {
          this.emit("outputTranscription", transcription.text);
          this.log(`server.transcription`, `output: ${transcription.text}`);
        }
      }
